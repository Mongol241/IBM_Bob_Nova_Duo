 ...
 */
function summariseFile(content: string, relPath: string): string {
...
export async function buildRepoContext(repoPath: string): Promise<string> {
  ...
  async function walk(dir: string): Promise<void> {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        if (!content || content.includes("<<<<<<<")) continue; // skip conflicted files
  ...
  const header =
    "=== REPOSITORY ARCHITECTURE (exported symbols per file) ===\n";
  ...
  if (lastNewline > header.length) cut = cut.slice(0, lastNewline);
 ...
 */
export function buildRepoContextFromStrings(files: Map<string, string>): string {
  const summaries: string[] = [];
