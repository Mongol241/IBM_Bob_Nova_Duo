...
export async function runApply(opts: {
  ...
  const tickets = parsed as Ticket[];
  ...
  if (startIdx === -1) {
    throw new Error(
      `Conflict marker "<<<<<<<" not found within ±${DRIFT} lines of line ${ticket.startLine} in ${ticket.file}. ` +
  ...
  if (endIdx === -1) {
    throw new Error(
      `Closing conflict marker ">>>>>>>" not found after line ${startIdx + 1} in ${ticket.file}.`
    );
  }
